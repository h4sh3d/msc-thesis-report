...
The sequence parser checks the first byte with the constant \texttt{0x30} and
...
filling. The big modulus is computed after parsing to accelerate encryption
and decryption.
...
multiplication are used to construct the signature component $s = D_{sk}(\mu)
...
The function implementation can also be split in four main parts (i) generate
